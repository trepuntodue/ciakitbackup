...
@section('page-detail-wrapper')
  ...
  @if (!$action && !request()->get('tipo'))
    ...
    <div class="step_1">
      <h2 class="text-xl uppercase">selezionare tipo</h2>
      <form id="cerca_release" method="post" action="{{ route('customer.release.store') }}" @submit.prevent="onSubmit"
        enctype="multipart/form-data">
        <div class="account-table-content mb-2">
          @csrf
          <div class="tipo-container mt-4 mb-4 flex gap-4 text-center">
            <input class="hidden" type="radio" id="tipo-video" name="tipo" value="video">
            <div class="tipo video bg-themeColor-500 flex rounded-md text-white">
              <label class="hover:bg-themeColor-400 w-full cursor-pointer px-4 py-2 font-bold uppercase text-black hover:rounded-md"
                for="tipo-video">
                Video
              </label>
            </div>
            <input class="hidden" type="radio" id="tipo-poster" name="tipo" value="poster">
            <div class="tipo poster bg-themeColor-500 flex rounded-md text-white">
              <label class="hover:bg-themeColor-400 w-full cursor-pointer px-4 py-2 font-bold uppercase text-black hover:rounded-md"
                for="tipo-poster">
                Poster
              </label>
          ...
          <div class="campo-cerca-container">
            <div class="cerca video mb-4 flex justify-center" style="display:none;">
              <label class="text-center">
                ...
                <input id="numero_catalogo" class="control" type="text" name="numero_catalogo">
              </label>
            </div>
            <div class="cerca poster mb-4 flex justify-center" style="display:none;">
              <label class="text-center">
                ...
                <input id="original_title" class="control" type="text" name="original_title"
          ...
          <div class="button-group btn-submit" style="display:none;">
            <input class="theme-btn" type="button" value="{{ __('shop::app.common.search') }}">
          </div>
        </div>
      </form>
    </div>
    <div class="step_2" style="display:none;">
      <h2 class="mb-4 text-center text-xl uppercase">{{ __('shop::app.common.seleziona_valore') }}</h2>
      <div class="results releases">
      </div>
      <div class="add-new">
        <form id="crea_nuovo" method="get" action="{{ route('customer.release.store') }}" @submit.prevent="onSubmit"
          enctype="multipart/form-data">
          <div class="account-table-content mb-2">
    ...
    <form method="post" action="{{ route('customer.release.store') }}" @submit.prevent="onSubmit"
      ...
      <div class="account-table-content mb-2">
        ...
        <div class="control-group" :class="[errors.has('master_id') ? 'has-error' : '']">
          ...
          <select data-mdb-filter="true" class="control js-example-basic-multiple" v-validate="'required'" id="master_id"
            name="master_id" data-vv-as="&quot;{{ __('admin::app.cinema.release.master_id') }}&quot;" required>
            <option value="">{{ __('shop::app.customer.account.release.create.linked_master') }}</option>  <!-- PWS#13-film PWS#finale -->
            @php
              $dt = false;
              if ($selected_master !== false) {
                  $dt = $selected_master->master_id;
              } // PWS#230101
            @endphp
            @foreach ($masters as $mas)
              <option {{ $mas->master_id == $dt ? 'selected' : '' }} value="{{ $mas->master_id }}">
                {{ $mas->master_maintitle }}</option> <!-- PWS#8-link-master -->
            @endforeach
          </select>

          <span class="control-error" v-text="errors.first('master_id')" v-if="errors.has('master_id')">
          </span>
        </div>

        <div class="control-group" :class="[errors.has('original_title') ? 'has-error' : '']">
          <label for="original_title"
            class="mandatory">{{ __('shop::app.customer.account.release.create.original_title') }}</label>
          @php
            $dt = '';
            if ($selected_master !== false) {
                $dt = $selected_master->master_maintitle;
            }
          @endphp
          <input class="control" type="text" name="original_title" value="{{ $dt }}"
            v-validate="'required'"
            data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.original_title') }}&quot;" required>
        ...
        <div class="control-group" :class="[errors.has('other_title') ? 'has-error' : '']">
          ...
          @php
            $dt = '';
            if ($selected_master !== false) {
                $dt = $selected_master->master_othertitle;
            }
          @endphp
          <input class="control" type="text" name="other_title" value="{{ $dt }}"
        ...
        <div class="control-group" :class="[errors.has('release_year') ? 'has-error' : '']">
          <label for="release_year"
            class="mandatory">{{ __('shop::app.customer.account.release.create.release_year') }}</label>
          @php
            $dt = '';
            if ($selected_master !== false) {
                $dt = $selected_master->master_year;
            }
          @endphp
          <input class="control" type="number" name="release_year" value="{{ $dt }}"
                        min="1900"
                        max="{{ date('Y'); }}"
            v-validate="'required'"
            data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.release_year') }}&quot;" required>

          <span class="control-error" v-text="errors.first('release_year')" v-if="errors.has('release_year')">
          </span>
        </div>

        @include ('shop::customers.account.release.country-state', [
        ...
        <div class="control-group" :class="[errors.has('release_distribution') ? 'has-error' : '']">
          <label for="release_distribution"
            class="mandatory">{{ __('shop::app.customer.account.release.create.release_distribution') }}</label>
          @php
            $dt = '';
            if ($selected_master !== false) {
                $dt = $selected_master->casaproduzione_nome;
            }
          @endphp
          <input type="text" class="control" name="release_distribution" value="{{ $dt }}"
            v-validate="'required'"
            data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.release_distribution') }}&quot;" required>

          <span class="control-error" v-text="errors.first('release_distribution')"
        ...
        @include ('shop::customers.account.release.releasetype', ['releasetypeCode' => old('releasetype')])
        <!-- PWS#02-23 -->
        ...
        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('formato') ? 'has-error' : '']" data-available_for="video">
            ...
            <select class="control styled-select" id="formato" type="text" name="formato"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.formato') }}&quot;" required>
            ...
            <span class="control-error" v-text="errors.first('formato')" v-if="errors.has('formato')">
            </span>
          </div>
        @endif

        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('aspect_ratio') ? 'has-error' : '']"
            data-available_for="video">
            <label for="aspect_ratio" class="mandatory">
              {{ __('shop::app.customer.account.release.create.aspect_ratio') }}
            </label>

            <select class="control styled-select" id="aspect_ratio" type="text" name="aspect_ratio"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.aspect_ratio') }}&quot;" required>
            ...
            <span class="control-error" v-text="errors.first('aspect_ratio')" v-if="errors.has('aspect_ratio')">
            </span>
          </div>
        @endif

        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('camera_format') ? 'has-error' : '']"
            data-available_for="video">
            <label for="camera_format" class="mandatory">
              {{ __('shop::app.customer.account.release.create.camera_format') }}
            </label>

            <select class="control styled-select" id="camera_format" type="text" name="camera_format"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.camera_format') }}&quot;" required>
            ...
            <span class="control-error" v-text="errors.first('camera_format')" v-if="errors.has('camera_format')">
            </span>
          </div>
        @endif
        ...
        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('region_code') ? 'has-error' : '']" data-available_for="video">
            ...
            <select class="control styled-select" id="region_code" type="text" name="region_code"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.region_code') }}&quot;" required>
            ...
            <span class="control-error" v-text="errors.first('region_code')" v-if="errors.has('region_code')">
            </span>
          </div>
        @endif
        ...
        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('tipologia') ? 'has-error' : '']" data-available_for="video">
            ...
            <select class="control styled-select" id="tipologia" type="text" name="tipologia"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.tipologia') }}&quot;" required>
            ...
            <span class="control-error" v-text="errors.first('tipologia')" v-if="errors.has('tipologia')">
            </span>
          </div>
        @endif

        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('canali_audio') ? 'has-error' : '']"
            data-available_for="video">
            <label for="canali_audio" class="mandatory">
              {{ __('shop::app.customer.account.release.create.canali_audio') }}
            </label>

            <select class="control styled-select" id="canali_audio" type="text" name="canali_audio"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.canali_audio') }}&quot;" required>
            ...
            <span class="control-error" v-text="errors.first('canali_audio')" v-if="errors.has('canali_audio')">
            </span>
          </div>
        @endif
        ...
        @if ($tipo == 'video')
          <div class="control-group" data-available_for="video">
            ...
            <label class="switch">
              <input type="hidden" name="subtitles" value="0"
                @if (old('subtitles') == 0) {{ 'checked' }} @endif>
              <input type="checkbox" id="subtitles" name="subtitles" value="1"
                @if (old('subtitles') == 1) {{ 'checked' }} @endif>
        ...
        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('durata') ? 'has-error' : '']" data-available_for="video">
            <label for="durata"
              class="mandatory">{{ __('shop::app.customer.account.release.create.durata') }}</label>
            @php
              $dt = '';
              if ($selected_master !== false) {
                  $dt = $selected_master->master_maintitle;
            ...
            @endphp
            <input class="control" type="number" name="durata" v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.durata') }}&quot;" required>

            <span class="control-error" v-text="errors.first('durata')" v-if="errors.has('durata')">
            </span>
          </div>
        @endif

        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('contenuti_speciali') ? 'has-error' : '']"
            data-available_for="video">
            <label for="contenuti_speciali"
              class="mandatory">{{ __('shop::app.customer.account.release.create.contenuti_speciali') }}</label>
            <textarea class="control"
                        style="resize:vertical"
                        rows="5"
                        name="contenuti_speciali"
                        v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.contenuti_speciali') }}&quot;" required></textarea>

            <span class="control-error" v-text="errors.first('contenuti_speciali')"
        ...
        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('numero_catalogo') ? 'has-error' : '']"
            data-available_for="video">
            <label for="numero_catalogo">{{ __('shop::app.customer.account.release.create.numero_catalogo') }}
              <span id="catalogo-tooltip" class="material-icons" data-toggle="tooltip" data-placement="top" title="{{ __('shop::app.customer.account.release.create.numero_catalogo_tooltip') }}">info</span> <!-- PWS#chiusura -->
            </label>
            <input class="control" type="text" name="numero_catalogo"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.numero_catalogo') }}&quot;"> <!-- PWS#chiusura -->

            <span class="control-error" v-text="errors.first('numero_catalogo')" v-if="errors.has('numero_catalogo')">
            </span>
          </div>
        @endif
        ...
        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('barcode') ? 'has-error' : '']" data-available_for="video">
            <label for="barcode">{{ __('shop::app.customer.account.release.create.barcode') }}</label>
            <input class="control" type="number" name="barcode"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.barcode') }}&quot;"> <!-- PWS#chiusura -->

            <span class="control-error" v-text="errors.first('barcode')" v-if="errors.has('barcode')">
            </span>
          </div>
        @endif
        ...
        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('crediti') ? 'has-error' : '']" data-available_for="video">
            <label for="crediti">{{ __('shop::app.customer.account.release.create.crediti') }}</label>
              <textarea
                        class="control"
                        style="resize:vertical"
                        rows="5"
                        name="crediti"
                        data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.crediti') }}&quot;"></textarea> <!-- PWS#chiusura -->

            <span class="control-error" v-text="errors.first('crediti')" v-if="errors.has('crediti')">
            </span>
          </div>
        @endif

        @if ($tipo == 'video')
          <div class="control-group" :class="[errors.has('release_description') ? 'has-error' : '']"
            data-available_for="video">
            <label for="release_description">{{ __('shop::app.customer.account.release.create.release_description') }}</label>
            <textarea class="control"
                        style="resize:vertical"
                        rows="5"
                        name="release_description"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.release_description') }}&quot;"></textarea> <!-- PWS#chiusura -->

            <span class="control-error" v-text="errors.first('release_description')"
        ...
        <!-- END PWS#02-23 -->
        @if ($tipo == 'poster')
          <div class="control-group" :class="[errors.has('poster_tipo') ? 'has-error' : '']"
            data-available_for="poster">
            <label for="poster_tipo"
              class="mandatory">{{ __('shop::app.customer.account.release.create.poster_tipo') }}</label>
            <select class="control styled-select" id="poster_tipo" type="text" name="poster_tipo"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.poster_tipo') }}&quot;" required>
        ...
        @if ($tipo == 'poster')
          <div class="control-group" :class="[errors.has('poster_formato') ? 'has-error' : '']"
            data-available_for="poster">
            <label for="poster_formato"
              class="mandatory">{{ __('shop::app.customer.account.release.create.poster_formato') }}</label>
            <select class="control styled-select" id="poster_formato" type="text" name="poster_formato"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.poster_formato') }}&quot;" required>
              <option value="">{{ __('shop::app.customer.account.release.create.select_poster_formato') }}
              </option>
        ...
        @if ($tipo == 'poster')
          <div class="control-group" :class="[errors.has('poster_misure') ? 'has-error' : '']"
            data-available_for="poster">
            <label for="poster_misure"
              class="mandatory">{{ __('shop::app.customer.account.release.create.poster_misure') }}</label>
            <select class="control styled-select" id="poster_misure" type="text" name="poster_misure"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.poster_misure') }}&quot;" required>
        ...
        @if ($tipo == 'poster')
          <div class="control-group" :class="[errors.has('illustratore') ? 'has-error' : '']">
            <label for="illustratore"
              class="mandatory">{{ __('shop::app.customer.account.release.create.illustratore') }}</label>
            <input class="control" type="text" name="illustratore" value="{{ $dt }}"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.illustratore') }}&quot;" required>

            <span class="control-error" v-text="errors.first('illustratore')" v-if="errors.has('illustratore')">
            </span>
          </div>
        @endif
        ...
        @if ($tipo == 'poster')
          <div class="control-group" :class="[errors.has('stampatore') ? 'has-error' : '']">
            <label for="stampatore"
              class="mandatory">{{ __('shop::app.customer.account.release.create.stampatore') }}</label>
            <input class="control" type="text" name="stampatore" value="{{ $dt }}"
              v-validate="'required'"
              data-vv-as="&quot;{{ __('shop::app.customer.account.release.create.stampatore') }}&quot;" required>

            <span class="control-error" v-text="errors.first('stampatore')" v-if="errors.has('stampatore')">
            </span>
          </div>
        @endif
...
@push('scripts')
  ...
  <script>
    ...
    function addToCollection(element_clicked, element_id) { // PWS#related-release-2
      if (config != undefined && config.routes != undefined && config.params != undefined && config.params.user_id != -
        1 && $('meta[name="_token"]').length > 0) {
        $.ajaxSetup({
          headers: {
