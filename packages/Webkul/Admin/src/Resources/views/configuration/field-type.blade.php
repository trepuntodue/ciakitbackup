...
@else
    <div class="control-group {{ $field['type'] }}" @if ($field['type'] == 'multiselect') :class="[errors.has('{{ $name }}[]') ? 'has-error' : '']" @else :class="[errors.has('{{ $name }}') ? 'has-error' : '']" @endif>
        ...
        @elseif ($field['type'] == 'language')
            ...
            @php $languageCode = core()->getConfigData($nameKey, $channel, $locale) ?? ''; @endphp
            ...
            <language
                ...
                :name = "'{{ $name }}'"
            ></language>
        @elseif ($field['type'] == 'releasetype')

            @php $releasetypeCode = core()->getConfigData($nameKey, $channel, $locale) ?? ''; @endphp

            <releasetype
                :releasetype_code = "'{{ $releasetypeCode }}'"
                :validations = "'{{ $validations }}'"
                :name = "'{{ $name }}'"
            ></releasetype>
...
@push('scripts')
    @if ($field['type'] == 'country')
        ...
        <script type="text/x-template" id="language-template">
            <div>
                <select type="text" v-validate="validations" class="control" :id="name" :name="name" v-model="language" data-vv-as="&quot;{{ __('admin::app.customers.customers.language') }}&quot;" @change="sendLanguageCode">
                    <option value=""></option>

                    @foreach (core()->language() as $language)
        ...
        <script>
            Vue.component('language', {
                ...
                props: ['language_code', 'name', 'validations'],
                ...
                mounted: function () {
                    this.language = this.language_code;
                    this.sendLanguageCode()
                },

                methods: {
                    sendLanguageCode: function () {
                        this.$root.$emit('languageCode', this.language)
                    },
                }
            });
        </script>
        ...
        <script type="text/x-template" id="releasetype-template">
            <div>
                <select type="text" v-validate="validations" class="control" :id="name" :name="name" v-model="releasetype" data-vv-as="&quot;{{ __('admin::app.customers.customers.releasetype') }}&quot;" @change="sendReleasetypeCode">
                    <option value=""></option>

                    @foreach (core()->releasetype() as $releasetype)

                        <option value="{{ $releasetype->code }}">{{ $releasetype->name }}</option>
        ...
        <script>
            Vue.component('releasetype', {
                ...
                props: ['releasetype_code', 'name', 'validations'],
                ...
                mounted: function () {
                    this.releasetype = this.releasetype_code;
                    this.sendReleasetypeCode()
                },

                methods: {
                    sendReleasetypeCode: function () {
                        this.$root.$emit('releasetypeCode', this.releasetype)
                    },
                }
            });
        </script>
